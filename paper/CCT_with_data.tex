...
For example, taking the average implies that raters are interchangeable and patients are independent -- the raters' bias or patients' offense is not taken into account.
...
An improvement over averaging the scores is to use Cultural Consensus Theory \parencite[CCT;][]{romney1986culture, batchelder1988test, batchelder2012cultural, erdfelder2020cognitive} \EJ{verwijs ook even naar de twee special issues ter ere van Bill} to construct an appropriate model for the scores that accounts for the hierarchical structure among patients, raters, and items.
...
%For the other variables, there are no apparent differences between non-violent and violent patients.\EJ{Nou ja...ik zou zeggen dat een intuitive assessment suggereert dat aggrevated assault, violent property crime, en moderate violence/property crime allicht wel risico-factoren zijn.}
...
%\EJ{Het gaat om een posterior verdeling over de true scores, toch? Misschien even expliciet aangeven.}
%, such as evaluating the mental state of a patient or usage as a risk indicator.
...
However, unlike the other coefficients shown, the IFTE coefficients should be interpreted as random effects, as they are multiplied with the estimated latent truth $\ItemTruth_{\Ipatient\Iitem}$ which varies across patients.\EJ{beetje onduidelijk dit}
...
Specifically, \autoref{fig:IFTE_aggregate} shows for each patient the posterior mean of the total impact of the IFTE for both measurements on the logit scale, which is given by $\sum_{t=1}^2\sum_{\Iitem=1}^{23}\ItemTruth_{\Ipatient\Iitem\Itime}\CovariateBeta_{\Iitem\Itime}$.\DB{Aangeven dat dit logit(kans op agressie) is.}
\begin{figure}[!ht]
    \centering
    \includesvg[width=.8\textwidth]{posterior_mean_IFTE_aggregate}
    \caption{Posterior mean of the effect of the total impact of the IFTE items for each patient. \EJ{geef vergelijking} Shape and color indicate whether patients showed violent behavior or not after T2.}
    \label{fig:IFTE_aggregate}
\end{figure}
\EJ{Wat staat er in de figuur? Even expliciet noemen dat violent patients een hogere posterior mean hebben dan non-violent patients.}
For some patients{preciezer graag}, the posterior mean of the impact of the aggregated IFTE is as large as the effect of prior violence.
However, the uncertainty of these estimates is large, and the average 95\% credible interval ranges from -8 to 8.\footnote{The large uncertainty here is inevitable, as the variance of a sum of random variables is often close to the sum of the variances of the individual parameters.}
...
It is evident from the model comparisons in the previous section that the sample mean of the IFTE items does not improve predictions of violent behavior.\EJ{Dit wekt verwarring, omdat het stijdig lijkt met hoe je Figure 8 hebt geinterpreteerd}
...
\autoref{fig:dataMisFitMean} visualizes the posterior mean of the latent truth $\ItemTruth_{\Ipatient\Iitem}$ against the standardized sample means for each patient and item. 
\begin{figure}[!ht]
    \centering
    \includesvg[width=.8\textwidth]{post_mean_lt_vs_item_sample_means}
...
Nevertheless, the uncertainty in the parameter estimates is too large to warrant strong conclusions and interpretations, other than the unsurprising result that prior history of violent behavior is a strong predictor of future violence.
...
For future data collection, it would be worthwhile to explore options to make the distribution of response scores unimodal.
This could be done through more clear instructions for the raters, or by collapsing infrequently used response categories. Such measures would facilitate modeling of the data and simultaneously reduce the likelihood of spurious patterns in the data.
...
A limitation of our approach is that patients' violent behavior is not a discrete event as we modeled it,\EJ{Je bedoelt: de informatie over wanneer de event precies plaatsvond wordt niet meegenomen} but rather a continuous process where the risk of violent behavior changes over time.
...
% Table Erwin send by email
% Nr & ITEM                                                 & Factor                                                                               \\ \hline
% 0  & Verandering                                          & -                                                                                    \\
% 1  & Probleeminzicht                                      & Beschermende factoren                                                                \\
% 2  & Psychotische symptomen                               & Probleemgedrag                                                                       \\
% 3  & Middelengebruik                                      & Probleemgedrag                                                                       \\
% 4  & Impulsief gedrag                                     & Probleemgedrag                                                                       \\
% 5  & Antisociaal gedrag                                   & Probleemgedrag                                                                       \\
% 6  & Vijandigheid                                         & Probleemgedrag                                                                       \\
% 7  & Sociale vaardigheden                                 & Resocialisatie vaardigheden                                                          \\
% 8  & Zelfredzaamheid                                      & Resocialisatie vaardigheden                                                          \\
% 9  & Meewerken aan de behandeling                         & Beschermende factoren                                                                \\
% 10 & Erkennen delict                                      & Beschermende factoren                                                                \\
% 11 & Copingvaardigheden                                   & Beschermende factoren                                                                \\
% 12 & Houden aan regels en voorwaarden                     & Probleemgedrag (Maar score spiegelen, zodat het schenden van regels wordt. 18-score) \\
% 13 & Arbeidsvaardigheden                                  & Resocialisatie vaardigheden                                                          \\
% 14 & Oriëntatie op antisociaal netwerk                    & Probleemgedrag                                                                       \\
% 15 & Evenwichtige dagindeling                             & Resocialisatie vaardigheden                                                          \\
% 16 & Financiële vaardigheden                              & Resocialisatie vaardigheden                                                          \\
% 17 & Medicatiegebruik                                     & Beschermende factoren                                                                \\
% 18 & Seksueel grensoverschrijdend gedrag                  & Probleemgedrag                                                                       \\
% 19 & Manipulatief gedrag                                  & Probleemgedrag                                                                       \\
% 20 & Vaardigheden tvv Middelengebruik                     & Beschermende factoren                                                                \\
% 21 & Vaardigheden tvv Fysiek Agressief Gedrag             & Beschermende factoren                                                                \\
...
%  	\begin{tabular}{>{\small}lll}
    ...
    % Nr & ITEM                                                 & Factor                  \\ 
\midrule
% Items in Schuringa et al., (2014) ordered as send by email
...
% Table Erwin send by email
% 0  & Verandering                                          & -                       \\
% 1  & Probleeminzicht                                      & Protective behaviors    \\
% 2  & Psychotische symptomen                               & Problematic behavior    \\
% 3  & Middelengebruik                                      & Problematic behavior    \\
% 4  & Impulsief gedrag                                     & Problematic behavior    \\
% 5  & Antisociaal gedrag                                   & Problematic behavior    \\
% 6  & Vijandigheid                                         & Problematic behavior    \\
% 7  & Sociale vaardigheden                                 & Resocialization Skills  \\
% 8  & Zelfredzaamheid                                      & Resocialization Skills  \\
% 9  & Meewerken aan de behandeling                         & Protective behaviors    \\
% 10 & Erkennen delict                                      & Protective behaviors    \\
% 11 & Copingvaardigheden                                   & Protective behaviors    \\
% 12 & Houden aan regels en voorwaarden                     & Problematic behavior    \\
% 13 & Arbeidsvaardigheden                                  & Resocialization Skills  \\
% 14 & Oriëntatie op antisociaal netwerk                    & Problematic behavior    \\
% 15 & Evenwichtige dagindeling                             & Resocialization Skills  \\
% 16 & Financiële vaardigheden                              & Resocialization Skills  \\
% 17 & Medicatiegebruik                                     & Protective behaviors    \\
% 18 & Seksueel grensoverschrijdend gedrag                  & Problematic behavior    \\
% 19 & Manipulatief gedrag                                  & Problematic behavior    \\
% 20 & Vaardigheden tvv Middelengebruik                     & Protective behaviors    \\
% 21 & Vaardigheden tvv Fysiek Agressief Gedrag             & Protective behaviors    \\
% Order in Schuringa et al., (2014)
    % Does the patient show problem insight?                                                                  &   Protective behaviors    &   HKT-R                   \\
    % Does the patient cooperate with your treatment?                                                         &   Protective behaviors    &   HKT-R                   \\
    % Does the patient admit and take responsibility for the crime(s)?                                        &   Protective behaviors    &   HKT-R                   \\
    % Does the patient show adequate coping skills?                                                           &   Protective behaviors    &   HKT-R                   \\
