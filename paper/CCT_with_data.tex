...
These are age (0-30, 31-40, 41-50, or 55+), treatment duration (0-2 years, 2-4 years, or 6+ years), diagnosis (Schizophrenia or other psychotic disorder, Personality disorder, Personality disorder B, Autism spectrum disorder, As 1 overig), Offence (Manslaughter, Heavy violence, Murder, Arson, Power with violence, Power/ medium violence), violent behavior before measurement 1, and violent behavior inbetween measurements 1 and 2.
...
We converted model probabilities for violent or non-violent behavior into binary predictions by comparing with 0.5. In other words, if a model prediction for observation $i$, $\hat{y}_i$, is larger than 0.5 then the predicted label is violent, otherwise it is non-violent.
The Brier score is defined using $N^{-1}\sum_{i=1}^N\left(\hat{y}_i-y_{i}\right)^2$, i.e., the mean squared error between the observed labels, $y$ and the model predictions.

\autoref{tb:predictions} shows the prediction accuracy and the Brier score averaged across the 20-folds.

\begin{table}[!ht]
    \caption{Predictive performance for multiple methods. Values are averaged across 20 cross validations, }
