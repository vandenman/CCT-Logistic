...
\node[latent, left = of y,                       ]     (g) 	{$\ItemCovariateBeta_{\Iitem}$};
...
{\tikzset{plate caption/.append style={below=0pt of #1.north east, xshift = -.5cm, yshift = .4cm}}
	\plate[text centered] {ni} {(x)(mu)(l)(y)}            {$\Tpatient$ patients };
}
...
\node[obsD, above = of y, minimum width = 1cm, xshift = -.3cm]	(zp)		{$\Covariate_{\Ipatient}$};
...
\node[latent, above = of l,                       ]     (g) 	{$\ItemCovariateBeta_{\Iitem}$};
\node[latent, above = of zp, xshift = .4cm                       ]     (h)  	{$\CovariateBeta_{\Icovariate}$};
...
{\tikzset{plate caption/.append style={below=0pt of #1.north east, yshift = -.1cm}}
	\plate[align=right] {ni} {(zp)(h)}            {$c$ Covariates};
}
...
%\node[obsD,   above = of b, yshift = +.65cm]		    (z)  	{$\RaterCovariate_{\Irater}$};
\node[obsD,   above = of b, yshift = -.65cm]		    (z)  	{$\Covariate_{\Irater}$};
%\node[obsD,   above = of l]  						    (w)		{$\Covariate_{\Ipatient}$};
\node[latent, right = of b, xshift = -.4cm]			    (muz)  	{$\mu_{\RaterShift}$};
%\node[latent, left  = of l, xshift = +.4cm]			    (mueta)	{$\mu_{\FactorScore}$};
