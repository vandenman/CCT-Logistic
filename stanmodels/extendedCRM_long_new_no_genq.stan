  ...
  if (!prior_only) {
    ...
    if (vectorized) {
      ...
      if (debug) {
        ...
        vector[n_observed] locations_tmp = exp(log_a)[idx_rater] .* to_vector(lt)[idx_longer_lt] + b[idx_rater];
        vector[n_observed] scales_tmp    = exp(log_E[idx_rater] + log_lambda[idx_item]);
        target += normal_lpdf(x | locations_tmp, scales_tmp);
      }
