...
real ordered_logistic_pmf_simplified_1(real location, real scale, real threshold_scale, real threshold_shift, real default_threshold_1) {
  return log1m_inv_logit((location - threshold_shift - threshold_scale * default_threshold_1) / scale);
}
real ordered_logistic_pmf_simplified_nc(real location, real scale, real threshold_scale, real threshold_shift, real default_threshold_nc) {
  return log_inv_logit((location - threshold_shift - threshold_scale * default_threshold_nc) / scale);
}
real ordered_logistic_pmf_simplified_other(int x, real location, real scale, real threshold_scale, real threshold_shift, vector default_thresholds) {
  return log_inv_logit_diff(
    (location - threshold_shift - threshold_scale * default_thresholds[x - 1]) / scale,
    (location - threshold_shift - threshold_scale * default_thresholds[x    ]) / scale
  );
...
real lpdf_ltm_sliced_observation_1(array[] int slice, int start, int end,
  array[] int idx_patient,
  array[] int idx_item,
  array[] int idx_rater,
  array[] int idx_time_point,
  ...
  for (o in start:end) {
    ...
    if (use_free_logistic_thresholds) {
      ...
      result += ordered_logistic_pmf_simplified_1(
          location, scale, 1.0, 0.0, free_thresholds[r][1]
        );
      ...
      result += ordered_logistic_pmf_simplified_1(
        location, scale, threshold_scale, threshold_shift, default_thresholds_1
      );
...
real lpdf_ltm_sliced_observation_nc(array[] int slice, int start, int end,
  array[] int idx_patient,
  array[] int idx_item,
  array[] int idx_rater,
  array[] int idx_time_point,
  ...
  for (o in start:end) {
    ...
    if (use_free_logistic_thresholds) {
      ...
      result += ordered_logistic_pmf_simplified_nc(
          location, scale, 1.0, 0.0, free_thresholds[r][nc]
        );
      ...
      result += ordered_logistic_pmf_simplified_nc(
        location, scale, threshold_scale, threshold_shift, default_thresholds_nc
      );
...
real lpdf_ltm_sliced_observation_other(array[] int slice, int start, int end,
  array[] int idx_patient,
  array[] int idx_item,
  array[] int idx_rater,
  array[] int idx_time_point,
  ...
  for (o in start:end) {
    ...
    if (use_free_logistic_thresholds) {
      ...
      result += ordered_logistic_pmf_simplified_other(
        x[o], location, scale, 1.0, 0.0, free_thresholds[r]
      );
...
data{
  ...
  // observed data -- WITHOUT 1 and nc
  array[n_observed] int<lower = 1, upper = nc> x;

  // indicator vectors -- WITHOUT 1 and nc
  ...
  // observations that equal 1
  int<lower = 1> n_observed_1;
  array[n_observed_1] int<lower = 1, upper = nr>             idx_rater_1;
  array[n_observed_1] int<lower = 1, upper = ni>             idx_item_1;
  array[n_observed_1] int<lower = 1, upper = np>             idx_patient_1;
  array[n_observed_1] int<lower = 1, upper = no_time_points> idx_time_point_1;

  // observations that equal nc
  int<lower = 1> n_observed_nc;
  array[n_observed_nc] int<lower = 1, upper = nr>             idx_rater_nc;
  array[n_observed_nc] int<lower = 1, upper = ni>             idx_item_nc;
  array[n_observed_nc] int<lower = 1, upper = np>             idx_patient_nc;
  array[n_observed_nc] int<lower = 1, upper = no_time_points> idx_time_point_nc;
...
model{
  ...
  target += reduce_sum(
    lpdf_ltm_sliced_observation_1, idx_patient_1, 1,
    idx_patient_1,
    idx_item_1,
    idx_rater_1,
    idx_time_point_1,
  ...
  target += reduce_sum(
    lpdf_ltm_sliced_observation_nc, idx_patient_nc, 1,
    idx_patient_nc,
    idx_item_nc,
    idx_rater_nc,
    idx_time_point_nc,
  ...
  target += reduce_sum(
    lpdf_ltm_sliced_observation_other, idx_patient, 1,
    idx_patient,
    idx_item,
    idx_rater,
    idx_time_point,
